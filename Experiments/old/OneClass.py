...
def WeightedLoss(outputs, R, c, nu, model, weights):
    dist   = torch.sum((outputs - c) ** 2, dim=1)
    scores = dist - R ** 2
    loss   = R ** 2 + (1 / nu) * torch.mean(torch.max(torch.zeros_like(scores), scores))
    
    scores = dist 
    regularisation_term = weighted_regularisation(weights, model)
    loss   = (1 / nu) * torch.mean(torch.max(torch.zeros_like(scores), scores)) + regularisation_term

    return loss


LossFns    = dict({"OneClass": OneClassLoss, "SoftBoundary": SoftBoundaryLoss, "Weighted": WeightedLoss})
...
class OneClassLayer(BaseNet):
    ...
    def __init__(self, params=None, hyperparams=None):
        ...
        self.reg_weights    = params['reg_weights']
